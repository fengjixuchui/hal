...
standard_graph_layouter_v3::standard_graph_layouter_v3(graph_context* context, QObject* parent) : graph_layouter(context, parent)
{
...
void standard_graph_layouter_v3::add(const QSet<u32> gates, const QSet<u32> nets)
{
...
void standard_graph_layouter_v3::remove(const QSet<u32> gates, const QSet<u32> nets)
{
    Q_UNUSED(gates)
    Q_UNUSED(nets)
}
...
const QString standard_graph_layouter_v3::description() const
{
...
void standard_graph_layouter_v3::create_boxes()
{
    // ZERO GATES
    int level_x = 0;
...
void standard_graph_layouter_v3::calculate_nets()
{
...
void standard_graph_layouter_v3::find_max_box_dimensions()
{
...
void standard_graph_layouter_v3::find_max_channel_lanes()
{
...
void standard_graph_layouter_v3::calculate_max_junction_spacing()
{
    for (const junction* j : m_junctions)
    {
        // LEFT
...
void standard_graph_layouter_v3::calculate_channel_spacing()
{
...
void standard_graph_layouter_v3::calculate_max_channel_dimensions()
{
...
void standard_graph_layouter_v3::calculate_gate_offsets()
{
...
void standard_graph_layouter_v3::place_gates()
{
    for (gate_box& box : m_boxes)
    {
...
void standard_graph_layouter_v3::update_scene_rect()
{
...
void standard_graph_layouter_v3::reset_roads_and_junctions()
{
...
void standard_graph_layouter_v3::draw_nets()
{
    for (const u32 id : m_nets)
    {
        std::shared_ptr<net> n = g_netlist->get_net_by_id(id);
...
void standard_graph_layouter_v3::clear_net_layout_data()
{
    for (const standard_graph_layouter_v3::road* r : m_h_roads)
        delete r;
    m_h_roads.clear();

    for (const standard_graph_layouter_v3::road* r : m_v_roads)
        delete r;
    m_v_roads.clear();

    for (const standard_graph_layouter_v3::junction* j : m_junctions)
        delete j;
    m_junctions.clear();
}

standard_graph_layouter_v3::gate_box standard_graph_layouter_v3::make_box(const u32 gate, const int x, const int y) const
{
    gate_box box;
    box.id = gate;
...
void standard_graph_layouter_v3::add_gate(const u32 gate_id, const int level)
{
...
bool standard_graph_layouter_v3::box_exists(const int x, const int y) const
{
    // VERIFIED
    for (const standard_graph_layouter_v3::gate_box& box : m_boxes)
        if (box.x == x && box.y == y)
            return true;
...
bool standard_graph_layouter_v3::h_road_jump_possible(const int x, const int y1, const int y2) const
{
    // VERIFIED
    int bottom_y = 0;
    int difference = 0;
...
bool standard_graph_layouter_v3::h_road_jump_possible(const standard_graph_layouter_v3::road* const r1, const standard_graph_layouter_v3::road* const r2) const
{
    // VERIFIED
    if (!(r1 && r2))
        return false;
...
bool standard_graph_layouter_v3::v_road_jump_possible(const int x1, const int x2, const int y) const
{
    // VERIFIED
    int right_x = 0;
    int difference = 0;
...
bool standard_graph_layouter_v3::v_road_jump_possible(const standard_graph_layouter_v3::road* const r1, const standard_graph_layouter_v3::road* const r2) const
{
    // VERIFIED
    if (!(r1 && r2))
        return false;
...
standard_graph_layouter_v3::road* standard_graph_layouter_v3::get_h_road(const int x, const int y)
{
    for (standard_graph_layouter_v3::road* r : m_h_roads)
        if (r->x == x && r->y == y)
            return r;

    standard_graph_layouter_v3::road* new_r = new road(x, y);
    m_h_roads.append(new_r);
    return new_r;
}

standard_graph_layouter_v3::road* standard_graph_layouter_v3::get_v_road(const int x, const int y)
{
    for (standard_graph_layouter_v3::road* r : m_v_roads)
        if (r->x == x && r->y == y)
            return r;
...
standard_graph_layouter_v3::junction* standard_graph_layouter_v3::get_junction(const int x, const int y)
{
    for (standard_graph_layouter_v3::junction* j : m_junctions)
        if (j->x == x && j->y == y)
            return j;

    standard_graph_layouter_v3::junction* new_j = new junction(x, y);
    m_junctions.append(new_j);
    return new_j;
}

qreal standard_graph_layouter_v3::h_road_height(const unsigned int lanes) const
{
...
qreal standard_graph_layouter_v3::v_road_width(const unsigned int lanes) const
{
    // LANES COUNTED FROM 1
...
qreal standard_graph_layouter_v3::scene_y_for_h_channel_lane(const int y, const unsigned int lane) const
{
    // VERIFIED
...
qreal standard_graph_layouter_v3::scene_x_for_v_channel_lane(const int x, const unsigned int lane) const
{
    // VERIFIED
    // LINES NUMBERED FROM 0
...
qreal standard_graph_layouter_v3::scene_x_for_close_left_lane_change(const int channel_x, unsigned int lane_change) const
{
    // VERIFIED
...
qreal standard_graph_layouter_v3::scene_x_for_far_left_lane_change(const int channel_x, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
qreal standard_graph_layouter_v3::scene_x_for_close_right_lane_change(const int channel_x, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
qreal standard_graph_layouter_v3::scene_x_for_far_right_lane_change(const int channel_x, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
qreal standard_graph_layouter_v3::scene_y_for_close_top_lane_change(const int channel_y, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
qreal standard_graph_layouter_v3::scene_y_for_far_top_lane_change(const int channel_y, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
qreal standard_graph_layouter_v3::scene_y_for_close_bottom_lane_change(const int channel_y, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
qreal standard_graph_layouter_v3::scene_y_for_far_bottom_lane_change(const int channel_y, unsigned int lane_change) const
{
    // VERIFIED
    // LANE CHANGES COUNTED FROM 0
...
void standard_graph_layouter_v3::store_max(QMap<T1, T2>& map, T1 key, T2 value)
{
    if (map.contains(key))
...
void standard_graph_layouter_v3::commit_used_paths(const standard_graph_layouter_v3::used_paths& used)
{
