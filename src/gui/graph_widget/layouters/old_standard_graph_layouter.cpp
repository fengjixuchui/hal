...
int old_standard_graph_layouter::s_max_level_size = 60;

old_standard_graph_layouter::old_standard_graph_layouter(graph_context* context, QObject* parent) : graph_layouter(context, parent),
    m_max_gate_height(0),
    m_max_gate_width(0),
    m_highest_level(0)
{
...
void old_standard_graph_layouter::layout()
{
    for (auto& g : g_netlist->get_gates())
    {
        graphics_gate* item = new advanced_graphics_gate(g);
        m_max_gate_height   = m_max_gate_height > item->get_height() ? m_max_gate_height : item->get_height();
        m_max_gate_width    = m_max_gate_width > item->get_width() ? m_max_gate_width : item->get_width();
        m_gate_map.insert(g, item);
    }
...
void old_standard_graph_layouter::reset()
{
    m_gate_map.clear();
    m_net_map.clear();
...
void old_standard_graph_layouter::add(const QSet<u32> gates, const QSet<u32> nets)
{
    Q_UNUSED(gates)
    Q_UNUSED(nets)
}

void old_standard_graph_layouter::remove(const QSet<u32> gates, const QSet<u32> nets)
{
    Q_UNUSED(gates)
    Q_UNUSED(nets)
}

void old_standard_graph_layouter::expand(const u32 from_gate, const u32 via_net, const u32 to_gate)
{
    Q_UNUSED(from_gate)
    Q_UNUSED(via_net)
    Q_UNUSED(to_gate)
...
const QString old_standard_graph_layouter::name() const
{
    return "Standard Layouter";
}

const QString old_standard_graph_layouter::description() const
{
    return "<p>The standard layouting algorithm</p>";
}

void old_standard_graph_layouter::compute_gate_levels()
{
    std::set<std::shared_ptr<gate>> unvisited = g_netlist->get_gates();
    std::queue<std::shared_ptr<gate>> dfs;

    for (const std::shared_ptr<net>& n : g_netlist->get_global_input_nets())
    {
        for (const endpoint& e : n->get_dsts())
        {
            std::shared_ptr<gate> dst = e.gate;

            if (unvisited.find(dst) != unvisited.end())
            {
                unvisited.erase(dst);
                dfs.push(dst);
    ...
    while (!dfs.empty())
    {
        std::shared_ptr<gate> g = dfs.front();
        int node_level          = m_gate_level.value(g) + 1;

        for (const endpoint& e : g->get_successors())
        {
            std::shared_ptr<gate> successor = e.gate;

            if (unvisited.find(successor) != unvisited.end())
            {
                unvisited.erase(successor);
                dfs.push(successor);
                m_gate_level.insert(successor, node_level);

                if (m_highest_level < node_level)
                    m_highest_level = node_level;
            }
        }
        dfs.pop();
    }

    // TODO DETERMINE HOW TO HANDLE UNVISITED GATES
    // ALSO SINGLE PASS NOT GOOD ENOUGH NEED TO CHECK FOR UNVISITED CHAINS
    for (const std::shared_ptr<gate>& g : unvisited)
    {
        ...
        for (const endpoint& e : g->get_successors())
        {
            if (unvisited.find(e.gate) != unvisited.end())
            {
                int level = m_gate_level.value(e.gate);

                if (lowest_successor_level > level)
...
void old_standard_graph_layouter::place_gates()
{
    int x = 0;
    for (std::list<graphics_gate*>& list : m_gate_layout)
    {
        int y = 0;
        for (graphics_gate*& g : list)
        {
            m_scene->addItem(g);
            g->setPos(m_max_gate_width * 2 * x, m_max_gate_height * 2 * y);
            ++y;
        }
...
void old_standard_graph_layouter::draw_global_nets()
{
...
void old_standard_graph_layouter::draw_separated_nets()
{
...
void old_standard_graph_layouter::old_compute_gate_levels()
{
    QList<std::shared_ptr<gate>> unvisited;

    for (const std::shared_ptr<gate>& g : g_netlist->get_gates())
        unvisited.append(g);
...
void old_standard_graph_layouter::compress_gate_levels()
{
...
void old_standard_graph_layouter::spin_gate_levels()
{
    //TODO
}

void old_standard_graph_layouter::old_place_gates()
{
    //channel width start
...
void old_standard_graph_layouter::compute_channel_width()
{
}
...
void old_standard_graph_layouter::draw_standard_nets()
{
...
void old_standard_graph_layouter::calculate_net(std::shared_ptr<net> n, int sub_channel)
{
